...
- Versão: a aba lista as milestones cujo título começa com `release/` ou `fix/` (case-insensitive) e, ao selecionar uma, as issues vinculadas. Tudo é lido ao vivo do GitLab por `GET /versao/milestones` e `GET /versao/issues?milestone=<título>` — não há tabela no banco. Filtro, mapeamento e ordenação ficam em `backend/src/comum/versao-gitlab.ts`, espelhados em `frontend/src/dominio/versao.ts`.
- As versões são milestones **do grupo** `mercantil`, não do projeto: a listagem usa `include_parent_milestones=true` (sem isso o projeto só devolve as `cliqq-centralizado::NNN`). Por isso as issues são buscadas por `projects/.../issues?milestone=<título>` — `projects/.../milestones/:id/issues` responde 404 para milestone herdada.
- A seção "Repositórios envolvidos" (`GET /versao/repositorios?milestone=<título>`) sai das **tasks**: cada task é um work item filho da issue, criada no projeto onde o código muda. A hierarquia só existe no GraphQL (`/api/graphql`), não no REST — a consulta e o agrupamento ficam em `backend/src/comum/repositorios-da-versao.ts`. A seção ignora os filtros da lista de issues de propósito: ela é o retrato da versão inteira.
- A seção "Versões" reaproveita a mesma resposta de `/versao/repositorios` (cada repositório traz os `iid` das issues) e cruza com as issues já carregadas na tela — não faz chamada nova ao GitLab. Uma issue desdobrada em vários projetos aparece em cada um deles, de propósito.
- Uma issue costuma ter mais de um `type::` (`type::bug` + `type::problemas-produção`), então `tipos` é lista; `state::` e `system::` continuam únicos.
- Desenvolvimento: a aba lista as milestones **abertas** (`state === 'active'`) `release/` ou `fix/` com todas as issues de cada uma, abertas e fechadas. Uma chamada só — `GET /desenvolvimento/milestones` monta a resposta buscando as issues de cada milestone em paralelo; o filtro, as contagens e a leitura das tags gravadas na descrição ficam em `backend/src/comum/desenvolvimento-gitlab.ts`. Sem agrupamento, sem ordenação por estado e sem filtros de propósito: a cor do `state::` é o que guia a leitura.
- A cor do `state::` é atribuída em sequência sobre a paleta `--estado-1..10` do `global.css` (`frontend/src/dominio/estados.ts`): a tela junta os estados que apareceram, ordena por nome e distribui as cores — assim dois estados nunca dividem a mesma cor, que é o ponto de colorir. Hash do nome foi tentado e descartado: com 6 estados reais, 3 colidiam na mesma cor. A paleta é montada uma vez para a tela inteira, não por milestone, e não há ordem implícita nas cores — não usar para indicar progresso.
