...
- A cor do `state::` sai de um hash do nome (`frontend/src/dominio/estados.ts`) sobre a paleta `--estado-1..8` do `global.css`: estado novo no GitLab já entra colorido e a cor não muda entre sessões. Não há ordem implícita nessas 8 cores — não usar para indicar progresso.
